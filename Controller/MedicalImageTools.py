...
class MedicalImageTools():
    ...
    def __init__(self):
        ...
        """
        self.__model = DicomModel(self.controller_listener)

        self.__root_view, self.__view = gui.show_main_view(self.controller_listener)
        self.__root_view.mainloop()

    def controller_listener(self, action_code: ListenerCode, **kwargs):
        """
        ...
        """
        if action_code == ListenerCode.SELECTED_IMAGES:
        ...
        elif action_code == ListenerCode.IMAGES_READ:
            self.__view.show_read_status(kwargs['status'])
            if kwargs['status'] == 1:
                self.__view.enable_widgets()
                self.__view.set_slider_limit(0, kwargs['upper_limit'])

                image1, image2, alpha_image = self.__model.get_tensor_images(axis=0,
                                                                             slice=0,
                                                                             alpha=100)
                self.__view.set_images(image1, image2, alpha_image)

        elif action_code == ListenerCode.RADIO_BUTTON_CHANGED:
            limit = self.__model.get_tensor_limit_for_axis(axis=kwargs['axis'])
            self.__view.set_slider_limit(0, limit)

            image1, image2, alpha_image = self.__model.get_tensor_images(axis=kwargs['axis'],
                                                                         slice=0,
                                                                         alpha=100)
            self.__view.set_images(image1, image2, alpha_image)

        elif action_code == ListenerCode.SLICE_SLIDER_CHANGED or action_code == ListenerCode.ALPHA_SLIDER_CHANGED:
            image1, image2, alpha_image = self.__model.get_tensor_images(axis=kwargs['axis'],
                                                                         slice=kwargs['slice'],
                                                                         alpha=kwargs['alpha'])
            self.__view.set_images(image1, image2, alpha_image)
